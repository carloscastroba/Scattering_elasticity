...
%M=8;      % number of points in the x-grid is N=2^M
...
%         w_sp_2=0*ui_sp_2;
        ...
        u_s_infty_1(i,j)=u_s_infty_1(i,j)*(1+rand*0.3);
        u_s_infty_2(i,j)=u_s_infty_2(i,j)*(1+rand*0.3);
        u_p_infty_1(i,j)=u_p_infty_1(i,j)*(1+rand*0.3);
        u_p_infty_2(i,j)=u_p_infty_2(i,j)*(1+rand*0.3);
...
%% Compute error
error=0.25*sqrt(h^2 * sum( sum( abs( (real(QB_11-q11)).^2 +(real(QB_12-q12)).^2 ...
    +(real(QB_21-q21)).^2 + (real(QB_22-q22)).^2 ))))
error_im=0.25*sqrt(h^2 * sum( sum( abs( (imag(QB_11-q11)).^2 +(imag(QB_12-q12)).^2 ...
    +(imag(QB_21-q21)).^2 +(imag(QB_22-q22)).^2 ))))

filename= join(["exp2_bs_noise03M",num2str(M,'%d'),".mat"],"");
save(filename);
end

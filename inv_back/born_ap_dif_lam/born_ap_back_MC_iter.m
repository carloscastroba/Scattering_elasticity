...
%% Use a finer mesh h/2 to compute scattering data 
clear all
...
nu_v=0.05:0.05:0.45;
EE_v=1:2:40;
for iii=1:length(nu_v)
    for jjj=1:length(EE_v)

        nu=nu_v(iii);
        EE=EE_v(jjj);
%% Lame coef
lam= EE*nu/((1+nu)*(1-2*nu)); %2;
mu= EE/(2*(1+nu)); %1;
rml=sqrt(2*mu+lam);
rm=sqrt(mu);
K=rml/rm;
...
for i=1:Nxi
    parfor j=1:Nxi
        ...
        [w_pp_1,w_pp_2]=sol_LS_fun2(lam,mu,rml*omega,x1,x2,Rz,R,M+1,ui_pp_1,ui_pp_2,q11,q12,q21,q22);
        [w_ps_1,w_ps_2]=sol_LS_fun2(lam,mu,2*rml/(K+1)*omega,x1,x2,Rz,R,M+1,ui_ps_1,ui_ps_2,q11,q12,q21,q22);
        [w_ss_1,w_ss_2]=sol_LS_fun2(lam,mu,rm*omega,x1,x2,Rz,R,M+1,ui_ss_1,ui_ss_2,q11,q12,q21,q22);
        [w_sp_1,w_sp_2]=sol_LS_fun2(lam,mu,2*rm/(1/K+1)*omega,x1,x2,Rz,R,M+1,ui_sp_1,ui_sp_2,q11,q12,q21,q22);
%         w_pp_1=0*ui_sp_2;
%         w_pp_2=0*ui_sp_2;
%         w_ps_1=0*ui_sp_2;
%         w_ps_2=0*ui_sp_2;
...
%         w_sp_2=0*ui_sp_2;
        ...
        u_pp_infty_1(i,j)=(gorro_Qtheta_1.*theta1+gorro_Qtheta_2.*theta2)*theta1...
            +(gorro_Qw_pp_1.*theta1+gorro_Qw_pp_2.*theta2)*theta1;
        u_pp_infty_2(i,j)=(gorro_Qtheta_1.*theta1+gorro_Qtheta_2.*theta2)*theta2...
            +(gorro_Qw_pp_1.*theta1+gorro_Qw_pp_2.*theta2)*theta2;
        ...
        u_ps_infty_1(i,j)=gorro_Qtheta_1-(gorro_Qtheta_1.*theta1+gorro_Qtheta_2.*theta2)*theta1...
            +gorro_Qw_ps_1-(gorro_Qw_ps_1.*theta1+gorro_Qw_ps_2.*theta2)*theta1;
        u_ps_infty_2(i,j)=gorro_Qtheta_2-(gorro_Qtheta_1.*theta1+gorro_Qtheta_2.*theta2)*theta2...
            +gorro_Qw_ps_2-(gorro_Qw_ps_1.*theta1+gorro_Qw_ps_2.*theta2)*theta2;
        % Amplitud p
        u_p_infty_1(i,j)=u_pp_infty_1(i,j)+u_ps_infty_1(i,j);
        u_p_infty_2(i,j)=u_pp_infty_2(i,j)+u_ps_infty_2(i,j);
        ...
        u_ss_infty_1(i,j)=gorro_Qthetao_1-(gorro_Qthetao_1.*theta1+gorro_Qthetao_2.*theta2)*theta1...
            +gorro_Qw_ss_1-(gorro_Qw_ss_1.*theta1+gorro_Qw_ss_2.*theta2)*theta1;
        u_ss_infty_2(i,j)=gorro_Qthetao_2-(gorro_Qtheta_1.*theta1+gorro_Qtheta_2.*theta2)*theta2...
            +gorro_Qw_ss_2-(gorro_Qw_ss_1.*theta1+gorro_Qw_ss_2.*theta2)*theta2;
        ...
        u_sp_infty_1(i,j)=(gorro_Qthetao_1.*theta1+gorro_Qthetao_2.*theta2)*theta1...
            +(gorro_Qw_sp_1.*theta1+gorro_Qw_sp_2.*theta2)*theta1;
        u_sp_infty_2(i,j)=(gorro_Qtheta_1.*theta1+gorro_Qtheta_2.*theta2)*theta2...
            +(gorro_Qw_sp_1.*theta1+gorro_Qw_sp_2.*theta2)*theta2;
        % Amplitud s
        u_s_infty_1(i,j)=u_ss_infty_1(i,j)+u_sp_infty_1(i,j);
        u_s_infty_2(i,j)=u_ss_infty_2(i,j)+u_sp_infty_2(i,j);

        % Add random noise to the scattering amplitudes
%         u_s_infty_1(i,j)=u_s_infty_1(i,j)*(1+rand*0.05);
%         u_s_infty_2(i,j)=u_s_infty_2(i,j)*(1+rand*0.05);
%         u_p_infty_1(i,j)=u_p_infty_1(i,j)*(1+rand*0.05);
%         u_p_infty_2(i,j)=u_p_infty_2(i,j)*(1+rand*0.05);
        % Transformada de Fourier de la aproximación de Born
        gorro_QB_11(i,j)=theta1*u_p_infty_1(i,j)+theta1o*u_s_infty_1(i,j);
        gorro_QB_21(i,j)=theta1*u_p_infty_2(i,j)+theta1o*u_s_infty_2(i,j);
        gorro_QB_22(i,j)=theta2*u_p_infty_2(i,j)+theta2o*u_s_infty_2(i,j);
        gorro_QB_12(i,j)=theta2*u_p_infty_1(i,j)+theta2o*u_s_infty_1(i,j);
        %stop
    end
end

% Grid points: x
[x1r,x2r,N,h] = create_grid(M, Rz);

% Aproximación de Born
 QB_11=(1/(h^2))*fftshift(ifftn(fftshift(gorro_QB_11)));
 QB_21=(1/(h^2))*fftshift(ifftn(fftshift(gorro_QB_21)));
 QB_22=(1/(h^2))*fftshift(ifftn(fftshift(gorro_QB_22)));
 QB_12=(1/(h^2))*fftshift(ifftn(fftshift(gorro_QB_12)));
...
% clf
% subplot(1,2,1)
% mesh(x1r,x2r,real(QB_11));
% subplot(1,2,2)
% mesh(x1r,x2r,imag(QB_11))
% 
% %% Matrix potential [ q11 q12 ; q21 q22 ]
 [q11,q12,q21,q22] = poten(x1r,x2r); % this is a matrix here
% 
% figure(1)
% clf
% subplot(1,2,1)
% mesh(x1r,x2r,real(q11));
% subplot(1,2,2)
% mesh(x1r,x2r,imag(q11))

%% Compute error
error(iii,jjj)=0.25*sqrt(h^2 * sum( sum( abs( (real(QB_11-q11)).^2 +(real(QB_12-q12)).^2 ...
    +(real(QB_21-q21)).^2 + (real(QB_22-q22)).^2 ))))
error_im(iii,jjj)=0.25*sqrt(h^2 * sum( sum( abs( (imag(QB_11-q11)).^2 +(imag(QB_12-q12)).^2 ...
    +(imag(QB_21-q21)).^2 +(imag(QB_22-q22)).^2 ))))


    end
end
%filename= join(["exp2_bsM",num2str(M,'%d'),".mat"],"");
save('exp_dif_lam_param.mat',"error","error_im","nu_v","EE_v");
[xx,yy]=meshgrid(nu_v,EE_v);
surf(xx,yy,error)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Funciones
...
function [w1,w2]=sol_LS_fun2(lam,mu,wvalue,x1,x2,Rz,R,M,ui1,ui2,q11,q12,q21,q22)
%% Main parameters 
% M  = number of points in the x-grid is N=2^M
% in this version we compute the solution in a finer grid M+1
% lam , mu = Lame coef
% wvalue = Energy
% (x1,x2) = Grid points

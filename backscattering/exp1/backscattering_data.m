...
M=9;      % number of points in the x-grid is N=2^M
...
u_pp_infty_1=zeros(N,N);
u_pp_infty_2=zeros(N,N);
u_ps_infty_1=zeros(N,N);
u_ps_infty_2=zeros(N,N);
u_sp_infty_1=zeros(N,N);
u_sp_infty_2=zeros(N,N);
u_ss_infty_1=zeros(N,N);
u_ss_infty_2=zeros(N,N);
u_s_infty_1=zeros(N,N);
u_s_infty_2=zeros(N,N);
u_p_infty_1=zeros(N,N);
u_p_infty_2=zeros(N,N);
gorro_QB_11=zeros(N,N);
gorro_QB_12=zeros(N,N);
gorro_QB_21=zeros(N,N);
gorro_QB_22=zeros(N,N);
...
clf
subplot(1,2,1)
mesh(x1,x2,real(QB_21));
subplot(1,2,2)
mesh(x1,x2,imag(QB_21))
